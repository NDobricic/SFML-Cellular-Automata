...
// ( gl_FragCoord.x >= float(int(tex_size.x) / 2)+0.5-32.0 )
...
void main() {
    ...
    float div = 1.0;
    float divi = floor((gl_FragCoord.x*div)/(tex_size.x)) + floor((gl_FragCoord.y*div)/(tex_size.y))*div;
    vec3 col = vec3( 0.0, 0.0, 0.0 );
    float dspace = (divi+1.0)/(div*div);
                dspace = (div == 1.0) ? 0.5 : dspace;
    float[VMX]     ubvn     = float[VMX]
    (0.158000, -0.082000, 0.130000, 0.004000, 0.524000, 0.345000, 0.085000, 0.446000, 0.257000, 0.375000, -0.030000, 0.300000, 0.013000, 0.268000, 0.528000, -0.012000, 0.546000, -0.074000, 0.224000, 0.119000, 0.365000, -0.008000, -0.023000, 0.384000, 0.518000, 0.136000, 0.531000, -0.035000, 0.063000, 0.108000, -0.023000, 0.122000 );
    float ezv = 0.0;
    float[VMX] dvmd;
    for(int i = 0; i < VMX; i++) { 
        dvmd[i] = ubvn[i] * 1.0
...
// Output
    if(div > 1.0) {
        res_g = (mod(gl_FragCoord.x,tex_size.x/div) <= 1.0 || (mod(gl_FragCoord.y,tex_size.y/div) <= 1.0)) ? 0.15 : res_r;
        res_b = (mod(gl_FragCoord.x,tex_size.x/div) <= 1.0 || (mod(gl_FragCoord.y,tex_size.y/div) <= 1.0)) ? 0.15 : res_r;
