...
//		div	 :	Integer-spaced number of toroidal divisions of the surface/medium
	...
	vec2 	dm 		= vec2(tex_size.x, tex_size.y);
    float 	divx	= dm[0] / div;
	float 	divy	= dm[1] / div;
	float 	pxo 	= 1.0 / dm[0];
	float 	pyo 	= 1.0 / dm[1];
...
//	Used to reseed the surface with noise
	vec4	fc = gl_FragCoord;
	float 	r0 = get_lump(fc[0], fc[1], 2.0, 19.0 + mod(time,17.0), 23.0 + mod(time,43.0));
	float 	r1 = get_lump(fc[0], fc[1], 24.0, 13.0 + mod(time,29.0), 17.0 + mod(time,31.0));
	float 	r2 = get_lump(fc[0], fc[1], 8.0, 13.0 + mod(time,11.0), 51.0 + mod(time,37.0));
...
//	----    ----    ----    ----    ----    ----    ----    ----
			...
			vec2 	dm 		= vec2(tex_size.x, tex_size.y);	//	Surface Dimensions
